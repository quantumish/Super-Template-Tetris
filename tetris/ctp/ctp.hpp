...
#else
#define CTP_INTERNAL_PRINT(x, y) \
	{ [[maybe_unused]] auto unused = (x) << static_cast<uint64_t>(y); }
#endif
...
/// Print tuple like.
template<uint64_t... Is, typename T, typename... Args>
...
template<uint64_t... Is, typename... Args>
...
public:
	...
	constexpr view(const T* first, U&& size) : data_{first}, size_{static_cast<uint64_t>(size)} {}

	constexpr view(const T* first, const T* last) : data_{first}, size_{static_cast<uint64_t>(last - first)} {}
